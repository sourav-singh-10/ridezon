...
const computeProfileCompletion = (user: {
    ...
    gender: string | null;
    avatar: string | null;
    verification: { status: string; trustScore: number } | null;
...
const buildMatchReasons = (
    ride: {
        ...
        creator: { verification: { status: string; trustScore: number } | null };
    },
    request: { origin: string; destination: string; departureTime: string },
) => {
...
const computeMatchScore = (
    ...
    if (normalize(ride.origin).includes(normalize(request.origin))) score += 25;
    else score += Math.round(originOverlap * 20);

    if (normalize(ride.destination).includes(normalize(request.destination))) score += 25;
    else score += Math.round(destinationOverlap * 20);

    const startDistance = approximateDistanceKm(
        ride.startLat,
        ride.startLng,
        request.originLat,
        request.originLng,
    );
    const endDistance = approximateDistanceKm(
        ride.endLat,
        ride.endLng,
    ...
    if (startDistance !== null) {
        if (startDistance <= 1.5) score += 10;
        else if (startDistance <= 5) score += 5;
    }
    if (endDistance !== null) {
        if (endDistance <= 1.5) score += 10;
        else if (endDistance <= 5) score += 5;
    }

    const departureDiffMinutes = Math.abs(
        new Date(ride.departureTime).getTime() - new Date(request.departureTime).getTime(),
    ) / (1000 * 60);

    if (departureDiffMinutes <= 15) score += 20;
    else if (departureDiffMinutes <= 45) score += 10;
...
export const getMobilityDashboard = async (req: AuthRequest, res: Response) => {
    try {
...
export const listEmergencyContacts = async (req: AuthRequest, res: Response) => {
    try {
        const userId = req.user?.id;
        if (!userId) return res.status(401).json({ message: "Unauthorized" });
...
export const listSavedCommutes = async (req: AuthRequest, res: Response) => {
    try {
        const userId = req.user?.id;
        if (!userId) return res.status(401).json({ message: "Unauthorized" });

        const commutes = await prisma.savedCommute.findMany({
            where: { userId },
            orderBy: { updatedAt: "desc" },
        });
...
export const createSavedCommute = async (req: AuthRequest, res: Response) => {
    try {
        const userId = req.user?.id;
        if (!userId) return res.status(401).json({ message: "Unauthorized" });

        const commute = await prisma.savedCommute.create({
            data: {
                userId,
...
export const deleteSavedCommute = async (req: AuthRequest, res: Response) => {
    try {
        const userId = req.user?.id;
        if (!userId) return res.status(401).json({ message: "Unauthorized" });

        const commute = await prisma.savedCommute.findUnique({
        ...
        await prisma.savedCommute.delete({
            where: { id: req.params.id },
        });
...
export const listRatings = async (req: AuthRequest, res: Response) => {
    try {
        const userId = req.user?.id;
        if (!userId) return res.status(401).json({ message: "Unauthorized" });
...
export const createSafetyAlert = async (req: AuthRequest, res: Response) => {
    try {
        const userId = req.user?.id;
        if (!userId) return res.status(401).json({ message: "Unauthorized" });
...
export const getRideMatches = async (req: AuthRequest, res: Response) => {
    try {
        const userId = req.user?.id;
        if (!userId) return res.status(401).json({ message: "Unauthorized" });

        const { origin, destination, departureTime } = req.query as {
            origin: string;
            destination: string;
            departureTime: string;
        };
        ...
        const matches = rides
            .map((ride) => {
                ...
                const originTextMatch =
                    normalize(ride.origin).includes(normalize(origin)) ||
                    normalize(origin).includes(normalize(ride.origin));
                const destinationTextMatch =
                    normalize(ride.destination).includes(normalize(destination)) ||
                ...
                const destinationAligned =
                    destinationTextMatch || destinationOverlap >= 0.55 || destinationGeoMatch;
...
export const seedDemoData = async (req: AuthRequest, res: Response) => {
    try {
        ...
        const existingCommutes = await prisma.savedCommute.count({
            where: { userId: currentUser.id },
        });

        if (!existingCommutes) {
            await prisma.savedCommute.createMany({
                data: [
                    {
                        userId: currentUser.id,
        ...
        const existingRideCount = await prisma.ride.count({
            where: {
                OR: [
